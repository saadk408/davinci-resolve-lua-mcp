...
- TypeScript: the `typescript-lsp` plugin (user scope) runs `typescript-language-server`, installed
  globally under nvm's Node, so it is on PATH only in a shell that sourced nvm. The project's
  `typescript` is 7.x, which ships no `tsserver` and no JavaScript API, so the server ignores the
  workspace install and falls back to the global `typescript`: keep that global on the 6.x line
  (`npm i -g typescript@6`, the last JavaScript-based release; a 6.0 project with the 7.0 defaults
  type-checks identically to 7, so the editor's diagnostics match `make typecheck` and CI, which run
  the native 7). The plugin's log line `Using Typescript version (bundled) 6.x.y` confirms the
  fallback. Measured 2026-09-22 with 7.0.2: TypeScript 7's own server (`tsc --lsp --stdio`) answers
  hover, definition, symbols and pull diagnostics, but pushes only empty `publishDiagnostics` on
  open and close and never a type error after an edit, a request, a save or a close, while Claude
  Code collects diagnostics from pushes only; a project `.lsp.json` pointing at it was tried and
  reverted (only one server per extension runs). Do not switch to it until it pushes. No headless
  mode; `npx tsc --noEmit` is the batch equivalent.
