...
- **Before calling a Resolve API method from Lua,** `hover` the method to see the declared signature. A placeholder class answers `fun(...): any`, which means "not declared yet": quote the method from the `.pyi` into `types/resolve_host.d.lua` first, then write the call. The LSP knows only what the types file declares; the `.pyi` stays the source of truth.
